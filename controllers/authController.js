...
const register = async (req, res) => {
  //If user has already signed up but has't verified email yet, send email verification
  const userForCheckingPurpose = await User.findOne({
    rsuEmail: req.body.rsuEmail,
  });
  if (userForCheckingPurpose) {
    if (!userForCheckingPurpose.isVerified) {
      const origin = "http://localhost:3000";
      await sendVerificationEmail(
        userForCheckingPurpose.name,
        userForCheckingPurpose.rsuEmail,
        userForCheckingPurpose.verificationToken,
        origin
      );
      res.json({
        msg: "Registration success! Please check your email to verify it",
        verificationToken: userForCheckingPurpose.verificationToken,
      });
      return;
    }
  }
...
const forgotPassword = async (req, res) => {
  ...
  if (user) {
    ...
    user.passwordTokenExpirationDate = new Date(
      Date.now() + passwordTokenExpirationDate
    );
    await user.save();
  }
...
const sendVerificationEmailLink = async (req, res) => {
  const { rsuEmail } = req.body;
  if (!rsuEmail) {
  ...
  if (user.isVerified) {
    throw new CustomError.BadRequestError("You have already verified.");
  }
  const origin = "http://localhost:3000";
  await sendVerificationEmail(
    user.name,
  ...
  res.json({
    msg: "Success! Please check your email to verify it.",
    verificationToken: user.verificationToken,
  });
};
